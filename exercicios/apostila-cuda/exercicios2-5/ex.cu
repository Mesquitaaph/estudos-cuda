...
__global__ void multMatriz(double *A, double *B, double *C) {
  ...
  __shared__ double A_sub[TAM_BLOCO];
  __shared__ double B_sub[TAM_BLOCO];

  // calcula o elemento C(i,j)
  double valor = 0;
  for(int passo = 0; passo < M; passo += blockDim.x) {
    A_sub[i_bloco*blockDim.x + j_bloco] = A[i*M + (j_bloco + passo)];
    B_sub[i_bloco*blockDim.x + j_bloco] = B[(i_bloco + passo)*L + j];

    __syncthreads();

    for(int k = 0; k < blockDim.x; k++) {
      valor += A_sub[i_bloco*blockDim.x + k] * B_sub[k*blockDim.x + j_bloco];
    }
